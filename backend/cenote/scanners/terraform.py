...
# Credentials mode: a real provider (no skip_* flags) so data sources and
# plan-time API validations actually run against AWS.
_CREDS_BASELINE_TF_TEMPLATE = """\
# Auto-generated by Cenote. Only injected when the uploaded code has no
# provider "aws" block, so terraform plan has something to initialize.
provider "aws" {
  region = "__REGION__"
...
def _inject_offline_provider(directory: Path, region: str, *, offline: bool = True) -> None:
    """Write a `provider "aws"` config into `directory` so plan can initialize.

    Offline mode (default) writes an `*_override.tf` that forces every skip_*
    flag onto the user's provider block (no AWS calls), plus a skip-everything
    baseline if the upload declares no provider.

    Credentials mode (`offline=False`) writes NO override — we WANT the
    provider to use the real credentials and validate — and only a region-only
    baseline when the upload has no provider block of its own.
    ...
    if not has_user_provider:
        template = _OFFLINE_BASELINE_TF_TEMPLATE if offline else _CREDS_BASELINE_TF_TEMPLATE
        (directory / "_cenote_baseline_provider.tf").write_text(
            template.replace("__REGION__", region)
        )
...
def _scrub(text: str, secrets: list[str]) -> str:
    ...
    misconfigured provider or a verbose error could."""
    for s in secrets:
        if s:
            text = text.replace(s, "***")
    return text
...
def run_terraform_plan_offline(
    ...
    - **Offline** (`aws_creds=None`, default): stub credentials + injected
      skip_* provider override so plan makes NO AWS calls. `data` sources and
      plan-time API validations that need real credentials will fail; the
      caller falls back to raw-HCL parsing.

    - **Credentials** (`aws_creds` given): the provided AWS keys/region are
      passed to terraform via the subprocess environment ONLY (never written
      to disk, never logged). No skip override is injected, so `data` sources
      (aws_caller_identity, aws_ami, …) and plan-time validations resolve for
      real. Still uses `-backend=false` and `-refresh=false`, so no remote
      state is touched and no managed resources are read — this is a plan,
      which never mutates infrastructure.
    ...
    offline = aws_creds is None
    region = (aws_creds or {}).get("AWS_REGION") or os.environ.get("AWS_REGION", "us-east-1")

    if offline:
        # Stub AWS env so the provider can initialize. The skip_* flags take
        # effect via the injected override file — AWS provider 5.x ignores
        # most of these env vars otherwise.
        env = {
            **os.environ,
        ...
        # Real credentials, env-only. Drop any inherited AWS_PROFILE so the
        # explicit keys win and we don't accidentally use the container's role.
        ...
        for key in ("AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN"):
            if aws_creds.get(key):
                env[key] = aws_creds[key]

    # Secrets to redact from any surfaced terraform output.
    secrets = (
        [env.get("AWS_SECRET_ACCESS_KEY", ""), env.get("AWS_SESSION_TOKEN", "")]
        if not offline
        else []
    )
    ...
    _inject_offline_provider(directory, region, offline=offline)
    ...
    def _run(args: list[str], step: str) -> subprocess.CompletedProcess[str]:
        log.info("terraform.offline.run", step=step, args=args)  # args carry no secrets
    ...
    if init.returncode != 0:
        ...
        raise TerraformError(_scrub("terraform init failed:\n" + "\n".join(tail), secrets))
    ...
    if plan.returncode != 0:
        ...
        raise TerraformError(_scrub("terraform plan failed:\n" + "\n".join(tail), secrets))

    show = _run(["show", "-json", str(plan_bin)], "show")
    if show.returncode != 0:
        raise TerraformError(_scrub(f"terraform show -json failed: {show.stderr[:500]}", secrets))
